...
<div class="container-fluid py-4">
    <h2 class="fw-bold mb-4"><i class="fas fa-chart-bar text-primary me-2"></i>Rekap Kehadiran Mahasiswa (Admin)</h2>
                ...
                <div class="col-auto">
                    ...
                    <select name="class_id" class="form-select form-select-sm" onchange="this.form.submit()">
                        ...
                        @foreach($allKelas as $kelas)
                            <option value="{{ $kelas->id }}" @selected($kelasF==$kelas->id)>
                                {{ $kelas->department->name ?? '' }} - Kelas {{ $kelas->nomor_kelas }} (Sem {{ $kelas->semester }})
                            </option>
                        @endforeach
                    </select>
                </div>
                <div class="col-auto">
                    <a href="{{ route('absensi.rekapAdmin') }}" class="btn btn-sm btn-outline-secondary">Reset</a>
                    <a href="{{ route('absensi.rekapAdmin', array_merge(request()->all(), ['export'=>'pdf'])) }}"
    ...
    <div class="card shadow-sm mb-4">
        <div class="card-header bg-primary text-white py-2 d-flex justify-content-between align-items-center">
            <div>
                ...
                &mdash; Dosen: {{ $item['schedule']->lecturer->name ?? '-' }}
            </div>
